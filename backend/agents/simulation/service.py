"""Simulation Agent(Agent G) 진입점.

파이프라인: 데이터 수집(data_collector) → 전처리(preprocessor) →
리스크 분류(risk_classifier) → LSTM 예측(model) → Monte Carlo(monte_carlo) →
결과 패키징(result_packager) → 프론트 전송.
"""
import logging
...
from agents.simulation.data_collector import collect_simulation_inputs, DEFAULT_B_DB_PATH
from agents.simulation.preprocessor import build_feature_table, InsufficientDataError
from agents.simulation.risk_classifier import classify_risk_factors
from agents.simulation.model import get_or_train_model, predict_distribution, apply_shock
from agents.simulation.monte_carlo import run_monte_carlo, N_PATHS, HORIZON
from agents.simulation.result_packager import package_result
